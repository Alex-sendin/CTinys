  ...
  $('.js-log-button').on('click', function (event) {
    ...
    $.ajax({
      ...
      .then((data) => {
        ...
        $('#log-modal-' + logId + '-request .log-modal-content-data')
          .removeClass('hidden')
          .html(prettyJson(data.log.request));
        $('#log-modal-' + logId + '-response .log-modal-content-data')
          .removeClass('hidden')
          .html(prettyJson(data.log.response));
        $('#log-modal-' + logId + '-context .log-modal-content-data')
          .removeClass('hidden')
          .html(prettyJson(data.log.context));
      });
  });
});
