...
$(document).ready(function () {
  ...
  initiateRefundButton.on('click', async () => {
    ...
    await window.merchantSDK.openRefundPage(
      clicktopay.locale,
      clicktopay.merchantReferenceId,
      clicktopay.gatewayTransactionId,
      isPartial,
      isPartial
        ? createRefundTransactionRequest(getRefundAmountInCents())
        : null
    );
  });
  ...
  showError = (message) => {
    ...
    $(
      '.clicktopay-card-body-refund-input-error .clicktopay-card-body-refund-input-error-text'
    ).text(message);
    $('input[name="clicktopay-refund-amount"]')
      .parents('.input-group')
      .addClass('clicktopay-card-body-refund-input-group-error');
    initiateRefundButton.attr('disabled', true);
  };
  ...
  hideError = () => {
    ...
    $('input[name="clicktopay-refund-amount"]')
      .parents('.input-group')
      .removeClass('clicktopay-card-body-refund-input-group-error');
    initiateRefundButton.attr('disabled', false);
  };

  getRefundAmountInCents = () =>
    parseFloat($('input[name="clicktopay-refund-amount"]').val()).toFixed(2) *
    100;
  isPartialRefund = () =>
    $('input[type=radio][name="clicktopay-refund"]:checked').val() !== 'full';
  ...
  window.addEventListener('message', function (messageEvent) {
    ...
    if (
      messageEvent.data.page !== 'refund' ||
      messageEvent.data.action !== 'data'
    ) {
      return;
    }
    ...
    $.growl.error({
      message: clicktopay.errors.somethingWentWrong,
      position: 'top-right',
      size: 'medium',
      duration: 10000,
    });
