...
const updatePhoneElement = (selector) => {
  ...
  if (document.querySelector('#thecheckout-address-delivery')) {
    if (!$(phoneElement).parents('label').find('.c2p-comment').length) {
      $(phoneElement)
        .parents('label')
        .append(clicktopay.click2payAdditionalPhoneInfoTemplate);
    }
  } else {
    phoneElement.insertAdjacentHTML(
      'afterend',
      clicktopay.click2payAdditionalPhoneInfoTemplate
    );
  }
  ...
  if (phoneElement.id !== 'delivery_phone_mobile') {
    if (document.querySelector(`label[for="${phoneElement.id}"]`)) {
      document.querySelector(`label[for="${phoneElement.id}"]`).textContent =
        clicktopay.newPhoneLabelValue;
    }
  }
};
...
const getPhoneElement = () => {
  ...
  if (
    ...
    document.querySelector('.js-address-form input[name=phone_mobile]')
  ) {
...
const getPhoneQuerySelectors = () => {
  let phoneElementName = getPhoneElement()
    ? getPhoneElement().getAttribute('name')
    : 'phone';
  ...
  emailQuerySelectors.forEach((selector) => {
    if (document.querySelector(selector)) {
      document
        ...
        .insertAdjacentHTML(
          'afterend',
          clicktopay.click2payAdditionalMailInfoTemplate
        );
    }
  });
