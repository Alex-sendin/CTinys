...
protected function getNewProducts()
{
    ...
    foreach ($newProducts as $rawProduct) {
        $productId = (int)$rawProduct['id_product'];
        $product = new Product($productId, false, $this->context->language->id);

        // Saltamos productos que no se pueden mostrar o no tienen combinaciones
        if (!$product->isAssociatedToShop() || !$product->hasAttributes()) {
            continue;
        }
        ...
        // Agrupamos los atributos por cada combinación
        $groupedCombinations = [];
        foreach ($combinations as $combination) {
            $id_pa = (int)$combination['id_product_attribute'];
            if (!isset($groupedCombinations[$id_pa])) {
                $groupedCombinations[$id_pa] = [
                    'id_product_attribute' => $id_pa,
                    'attributes' => [],
                ];
            }
            $groupedCombinations[$id_pa]['attributes'][] = [
                'group_name' => $combination['group_name'],
                'name' => $combination['attribute_name'],
            ];
        }

        // Construimos nuestro propio array para la plantilla, sin usar el presentador
        foreach($groupedCombinations as $comboData) {
            $id_product_attribute = $comboData['id_product_attribute'];
            $id_image = null;

            // Buscamos la imagen de esta combinación
            if (isset($combination_images[$id_product_attribute][0]['id_image'])) {
                $id_image = $combination_images[$id_product_attribute][0]['id_image'];
            } else {
                // Si no tiene imagen específica, usamos la de portada del producto
                $cover = Product::getCover($productId);
                $id_image = $cover['id_image'];
            }
            
            $homeImageTypeName = ImageType::getFormattedName('home');

            // Añadimos la combinación con todos sus datos listos para usar
            $products_for_template[] = [
                'id_product' => $productId,
                'id_product_attribute' => $id_product_attribute,
                'name' => $rawProduct['name'],
                // Obtenemos la URL directa a la combinación
                'url' => $this->context->link->getProductLink($product, null, null, null, $this->context->language->id, null, $id_product_attribute, false, false, true),
                // Obtenemos la URL de la imagen de la combinación
                'image_url' => $this->context->link->getImageLink($rawProduct['link_rewrite'], $id_image, $homeImageTypeName),
                // Obtenemos el precio final de la combinación
                'price' => Product::getPriceStatic($productId, true, $id_product_attribute),
